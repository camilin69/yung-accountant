      ...
      <div className="w-full max-w-md flex flex-col max-h-[85vh] rounded-[2rem] overflow-hidden glass-aero animate-scale-in">
        ...
        <div className="flex-1 overflow-y-auto modal-scroll p-5 space-y-5">
          ...
          {(formData.type === 'bank_account' || formData.type === 'credit_card' || formData.type === 'debit_card') && (
            <>
              <div>
                <label className="block text-xs font-medium tracking-[0.04em] uppercase mb-1.5" style={{ color: 'var(--theme-text-tertiary)' }}>Bank Name (Optional)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-medium tracking-[0.04em] uppercase mb-1.5" style={{ color: 'var(--theme-text-tertiary)' }}>Last 4 Digits (Optional)</label>
                <input
                  type="text"
                  maxLength={4}
        ...
        <div style={{ borderTop: '1px solid var(--theme-border-dark)' }}>
          <div className="flex gap-3 p-5">
            ...
            <button onClick={onSubmit} disabled={!formData.name.trim()}
              ...
              <Save className="w-4 h-4" /> {editingWallet ? 'Update' : 'Create'}
            </button>
          </div>
        </div>
      </div>
