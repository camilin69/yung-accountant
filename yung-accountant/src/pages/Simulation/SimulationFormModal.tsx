          ...
          <div className="p-5 space-y-5">
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <div className="w-36">
                  <CustomSelect value={formData.period} onChange={(value) => onPeriodChange(value as 'day' | 'week' | 'month')} options={periodOptions} placeholder="Period" />
                </div>
              </div>
              {formData.amount > 0 && formData.startDate && formData.endDate && (
                <div className="mt-2 p-3 rounded-[1rem] glass-sm">
                  <p className="text-[10px] font-medium" style={{ color: 'var(--theme-text-tertiary)' }}>
                    Total for period: <span style={{ color: 'var(--theme-primary)' }}>{formatCurrency(modalTotalPreview())}</span>
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="pt-2" style={{ borderTop: '1px solid var(--theme-border-dark)' }}>
              <p className="text-xs font-medium tracking-[0.04em] uppercase mb-3" style={{ color: 'var(--theme-text-tertiary)' }}>Duration (editable)</p>
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  <label className="block text-xs font-medium tracking-[0.04em] uppercase mb-1.5" style={{ color: 'var(--theme-text-tertiary)' }}>Months</label>
                  <input 
        ...
        <div style={{ borderTop: '1px solid var(--theme-border-dark)' }}>
          <div className="flex gap-3 p-5">
            ...
            <button onClick={onSubmit} className="flex-1 px-4 py-2.5 rounded-2xl text-white text-sm font-medium transition-all duration-500 hover:-translate-y-1 flex items-center justify-center gap-2"
              ...
              <Save className="w-4 h-4" /> {editingTransaction ? 'Update' : 'Create'}
            </button>
          </div>
        </div>
      </div>
