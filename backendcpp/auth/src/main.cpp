...
private:
    ...
    void handle_update_me(http::response<http::string_body>& res) {
        ...
        if (!userInfo.isValid) {
            ...
            emitAuthEvent("update_profile_failed", "", "", status_code,
                         {{"reason", "invalid_token"}});
            return;
        }
        ...
        try {
            ...
            if (!userOpt) {
                ...
                emitAuthEvent("update_profile_failed", userInfo.mongoId, userInfo.email, status_code,
                             {{"reason", "user_not_found"}});
                return;
            }
            ...
            if (obj.contains("email")) {
                ...
                res.result(status_code);
                json::object error;
                error["error"] = "No se puede cambiar el email";
                res.body() = json::serialize(error);
                
                emitAuthEvent("update_profile_failed", userOpt->id, userInfo.email, status_code,
            ...
            bool mongoOk = UserService::getInstance().updateUser(userOpt->id, firstName, lastName, age);
            bool keycloakOk = keycloakClient->updateUser(userOpt->keycloakId, firstName, lastName);
            ...
            if (mongoOk) {
                ...
                response["lastName"] = lastName;
                response["age"] = age;
                response["keycloakUpdated"] = keycloakOk;
                res.body() = json::serialize(response);
                
                emitAuthEvent("user_updated", userOpt->id, userInfo.email, status_code,
                             {{"old_firstName", oldFirstName}, {"old_lastName", oldLastName}, {"old_age", oldAge},
                              {"new_firstName", firstName}, {"new_lastName", lastName}, {"new_age", age}});
            } else {
                int status_code = static_cast<int>(http::status::internal_server_error);
                res.result(status_code);
                json::object error;
                error["error"] = "Error actualizando usuario";
                res.body() = json::serialize(error);
                
                emitAuthEvent("update_profile_failed", userOpt->id, userInfo.email, status_code,
                             {{"reason", "mongodb_update_failed"}});
            }
...
int main() {
    try {
        ...
        if (!keycloakRealm) keycloakRealm = "cuenta-confiable";
        ...
        if (!mongoUri) mongoUri = "mongodb://admin:secret123@mongodb:27017";
        if (!mongoDb) mongoDb = "cuenta-confiable";
        
        Database::getInstance().connect(mongoUri, mongoDb);
