...
private:
    ...
    void handle_request() {
        ...
        try {
            ...
            if (req_.method() == http::verb::get && target == "/transactions") handle_get_transactions(res);
            else if (req_.method() == http::verb::get && target.find("/transactions/") == 0) handle_get_transaction(res);
            else if (req_.method() == http::verb::post && target == "/transactions") handle_create_transaction(res);
            else if (req_.method() == http::verb::put && target.find("/transactions/") == 0) handle_update_transaction(res);
            else if (req_.method() == http::verb::delete_ && target.find("/transactions/") == 0) handle_delete_transaction(res);
    ...
    void handle_get_transactions(http::response<http::string_body>& res) {
        ...
        if (!userInfo.isValid) { res.result(http::status::unauthorized); res.body() = json::serialize(json::object{{"error", "Token inválido"}}); return; }
        auto transactions = TransactionService::getInstance().getTransactionsByUser(userInfo.postgresId);
        json::array arr;
        for (const auto& t : transactions) {
            json::object obj;
            obj["id"] = t.id; obj["amount"] = t.amount; obj["description"] = t.description;
            obj["date"] = t.date; obj["walletId"] = t.walletId; obj["categoryId"] = t.categoryId;
            obj["categoryName"] = t.categoryName;
            json::array tags; for (const auto& tag : t.tags) tags.push_back(json::value(tag));
            obj["tags"] = tags; obj["createdAt"] = t.createdAt;
            arr.push_back(obj);
        }
        res.result(http::status::ok); res.body() = json::serialize(arr);
        emitTransactionEvent("get_transactions", userInfo.postgresId, "", 200, {{"count", static_cast<int64_t>(transactions.size())}});
    }
    ...
    void handle_get_transaction(http::response<http::string_body>& res) {
        ...
        std::string id = std::string(req_.target().begin(), req_.target().end()).substr(14);
        auto t = TransactionService::getInstance().getTransactionById(id, userInfo.postgresId);
        if (!t) { res.result(http::status::not_found); return; }
        json::object obj;
        obj["id"] = t->id; obj["amount"] = t->amount; obj["description"] = t->description;
        obj["date"] = t->date; obj["walletId"] = t->walletId; obj["categoryId"] = t->categoryId;
        obj["categoryName"] = t->categoryName;
    ...
    void handle_create_transaction(http::response<http::string_body>& res) {
        ...
        try {
            ...
            if (obj.contains("tags")) for (const auto& tag : obj.at("tags").as_array()) t.tags.push_back(boost::json::value_to<std::string>(tag));
            
            auto created = TransactionService::getInstance().createTransaction(t);
            if (!created) { res.result(http::status::internal_server_error); return; }
...
int main() {
    try {
        unsigned short port = std::getenv("SERVER_PORT") ? std::stoi(std::getenv("SERVER_PORT")) : 8087;
