...
private:
    ...
    void handle_request() {
        ...
        try {
            ...
            if (req_.method() == http::verb::get && target == "/wallets") handle_get_wallets(res);
            else if (req_.method() == http::verb::post && target == "/wallets") handle_create_wallet(res);
            else if (req_.method() == http::verb::put && target.find("/wallets/") == 0) handle_update_wallet(res);
            else if (req_.method() == http::verb::delete_ && target.find("/wallets/") == 0) handle_delete_wallet(res);
    ...
    void handle_get_wallets(http::response<http::string_body>& res) {
        ...
        for (const auto& w : wallets) {
            ...
            obj["bankName"] = w.bankName; obj["lastFourDigits"] = w.lastFourDigits;
            obj["color"] = w.color; obj["icon"] = w.icon;
        ...
        emitWalletEvent("get_wallets", userInfo.postgresId, "", 200, {{"count", static_cast<int64_t>(wallets.size())}});
    }
    ...
    void handle_create_wallet(http::response<http::string_body>& res) {
        ...
        try {
            ...
            if (obj.contains("currentBalance")) {
                auto& v = obj.at("currentBalance");
                if (v.is_int64()) w.currentBalance = static_cast<double>(v.as_int64());
                else w.currentBalance = v.as_double();
            } else {
                w.currentBalance = 0;
            }
            w.isActive = true;
            ...
            if (!created) { res.result(http::status::internal_server_error); res.body() = json::serialize(json::object{{"error", "Error"}}); return; }
            res.result(http::status::created);
            res.body() = json::serialize(json::object{{"id", created->id}, {"message", "Wallet creada"}});
            emitWalletEvent("wallet_created", userInfo.postgresId, created->id, 201, {{"name", w.name}, {"type", w.type}});
        } catch (const std::exception& e) { res.result(http::status::bad_request); res.body() = json::serialize(json::object{{"error", e.what()}}); }
    }
    
    void handle_update_wallet(http::response<http::string_body>& res) {
        auto userInfo = verifyAndGetUser(req_);
        if (!userInfo.isValid) { res.result(http::status::unauthorized); res.body() = json::serialize(json::object{{"error", "Token inválido"}}); return; }
        std::string id = std::string(req_.target().begin(), req_.target().end()).substr(9);
        try {
            auto jv = json::parse(req_.body());
...
int main() {
    try {
        unsigned short port = std::getenv("SERVER_PORT") ? std::stoi(std::getenv("SERVER_PORT")) : 8086;
